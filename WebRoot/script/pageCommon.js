...
function openNewWindow() {
	if (document.homeForm.newWindow.value != "") {
		var varOption = "toolbar=no,location=no,status=yes,menubar=no,resizable=yes,scrollbars=yes,width="
				...
				+ ",left=0,top=0";
		window
				.open(document.homeForm.newWindow.value, 'autoOpenWin',
						varOption);
...
function removeLocationAddress(html) {
	...
	while (findSite > -1) {
		html = html.substring(0, findSite)
				+ html.substring(findSite + hostAdd.length);
...
function saveTip(message) {
	...
	if(document.charpter.cname.value=="")
	 ...
	 alert(document.charpter.cname.value);
	alert(message);
	return false;
...
/*
 * 检查是否选择了数据 @param bAllowSelectMultiple 布尔型，指定是否允许选中多条数据
 */
...
/*
 * 根据参数值数组，拼接成请求参数字符串，并替换相应link的href属性为新拼接的url。 所使用的基地址为linkElement指定的href属性值
 * @param linkElement 相应的<a>元素对象 @param paramName 参数名 @param paramValueArrya
 * 参数值数组
 */
 ...
 * 
 * url String 被加载的HTML文档的 URL 地址 name String 打开的窗口的名字 width Number 窗口的宽度 height
 * Number 窗口的高度 scrollbars Boolean 是否有滚动条
 */
function myOpenWindow(url, width, height, name, scrollbars) {
	...
	var features = "left=" + left + ",top=" + top + ",width=" + width
			...
			+ (scrollbars ? "yes" : "no") + ",center=yes" + ",resizable=yes"
			+ ",status=yes";
